...
# /heroes/movies
resource "aws_api_gateway_resource" "heroes_movies" {
  rest_api_id = aws_api_gateway_rest_api.marvel.id
  parent_id   = aws_api_gateway_resource.heroes.id
  path_part   = "movies"
}

resource "aws_api_gateway_method" "get_heroes_movies" {
  rest_api_id   = aws_api_gateway_rest_api.marvel.id
  resource_id   = aws_api_gateway_resource.heroes_movies.id
  http_method   = "GET"
  authorization = "NONE"
}

resource "aws_api_gateway_integration" "lambda_get_heroes_movies_integration" {
  rest_api_id             = aws_api_gateway_rest_api.marvel.id
  resource_id             = aws_api_gateway_resource.heroes_movies.id
  http_method             = aws_api_gateway_method.get_heroes_movies.http_method
  type                    = "AWS_PROXY"
  integration_http_method = "POST"
  uri                     = aws_lambda_function.get_heroes_by_movies.invoke_arn
}
